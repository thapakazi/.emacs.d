...
#+END_SRC
* Recentf Settings
  Configure recentf (mode enabled in basics.org with the Vertico stack).

  #+begin_src emacs-lisp
;; Keep more recent files in history
(setq recentf-max-saved-items 500)

;; Quick access to recent files
(global-set-key (kbd "C-x ,") 'consult-recent-file)
  #+end_src
...
#+begin_src emacs-lisp
  (use-package general
    ...
    ;; Define your keybindings (using consult)
    (my-leader-keys
...
We install `asdf.el` directly from the source and enable it immediately. This ensures that when Emacs looks for `python` or `black`, `go` or any asdf managed things, it uses the `asdf` shims.

#+BEGIN_SRC emacs-lisp
(use-package asdf
  :ensure (:host github :repo "tabfugnic/asdf.el")
  :config
  (asdf-enable))
#+END_SRC
